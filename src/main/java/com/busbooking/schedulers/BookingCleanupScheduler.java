...
/**
 * Scheduled service for automatic cleanup of expired PENDING bookings. This restores seat inventory
 * if a user fails to confirm payment within the 10-minute window.
 */
@Component
...
public class BookingCleanupScheduler {
    ...
    /**
     * CRITICAL: Runs every minute to check for expired bookings. The cron expression "0 * * * * *"
     * means: at second 0 of every minute.
     */
    @Scheduled(cron = "0 * * * * *")
    @Transactional
    public void cleanupExpiredBookings() {
        ...
        List<Booking> expiredBookings =
                bookingRepository.findByStatusAndExpirationTimeBefore(BookingStatus.PENDING, now);
        ...
        log.warn(
                "Found {} expired PENDING bookings. Processing cleanup...", expiredBookings.size());
        ...
        for (Booking booking : expiredBookings) {
            try {
                ...
                // In a real system, we'd delete the corresponding seat reservation from the Redis
                // BookingCache here.
                log.info(
                        "Booking ID: {} EXPIRED. Seats are now considered released for re-booking.",
                        booking.getId());

            } catch (Exception e) {
                // Structured Logging for failure
                log.error(
                        "Failed to cleanup expired booking ID: {}. Error: {}",
                        booking.getId(),
                        e.getMessage());
            }
        }
