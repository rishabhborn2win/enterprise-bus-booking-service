...
public class SeatAvailabilityService {
    ...
    public SeatAvailabilityResponse getSegmentAvailability(Long scheduleId, Long sourceStopId, Long destinationStopId) {
        // 1. Get Stop Orders for the requested query segment (X -> Y)
        Integer queryStartOrder = scheduleStopRepository.findStopOrderByScheduleIdAndStopId(scheduleId, sourceStopId)
            .orElseThrow(() -> new IllegalArgumentException("Source stop not valid for schedule."));
        Integer queryEndOrder = scheduleStopRepository.findStopOrderByScheduleIdAndStopId(scheduleId, destinationStopId)
            .orElseThrow(() -> new IllegalArgumentException("Destination stop not valid for schedule."));

        // Validation: X must be before Y
        if (queryStartOrder >= queryEndOrder) { throw new IllegalArgumentException("Invalid segment: Source must precede destination."); }
        ...
        for (Object[] segment : bookedSegments) {
            ...
            boolean overlaps = (bookedStartOrder < queryEndOrder) && (bookedEndOrder > queryStartOrder);
        ...
        Map<String, SeatClass> seatMap = allSeats.stream()
            .collect(Collectors.toMap(Seat::getSeatNumber, Seat::getSeatClass));
            
        Set<String> bookedSeatNumbers = allSeats.stream()
            .filter(seat -> unavailableSeatIds.contains(seat.getId()))
            .map(Seat::getSeatNumber)
            .collect(Collectors.toSet());

        Set<String> availableSeatNumbers = allSeats.stream()
            .map(Seat::getSeatNumber)
            .collect(Collectors.toSet());
            
        availableSeatNumbers.removeAll(bookedSeatNumbers);

        return new SeatAvailabilityResponse(totalCapacity, bookedSeatNumbers.stream().toList(), availableSeatNumbers.stream().toList(), seatMap);
    }
}