...
@Tag(name = "Admin Management", description = "Endpoints for managing Buses, Routes, and Schedules (CRUD + Sync)")
public class AdminController {

    private final AdminService adminService;
    private final SyncService syncService;

    @Operation(summary = "Create or Update Schedule", description = "Adds a new schedule and its sequential stops in MySQL, triggering an immediate sync to Elasticsearch.")
    @PostMapping("/schedule")
    public ResponseEntity<ScheduleResponseDto> addSchedule(@RequestBody ScheduleRequestDto request) {
    ...
    @Operation(summary = "Manually Trigger Full Sync", description = "Pulls ALL active schedules from MySQL and rebuilds the Elasticsearch index. Use for initialization or recovery.")
